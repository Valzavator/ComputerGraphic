...
public class ReadingImageFromFile {
    ...
    // метод для читання файлу з зображенням та виведення інформації про зчитаний файл, який
    // приймає на вхід назву файлу з зображенням у форматі BMP
    public static void loadBitmapImage(String filename) throws IOException {
        int line;
        BufferedInputStream reader = new BufferedInputStream(new FileInputStream(filename)); // потік для читання
        BufferedOutputStream writer = new BufferedOutputStream(new FileOutputStream("primer_bmp.txt")); // потік для запису зчитаних зображень про пікселі у ASCII кодах
        ...
        BufferedInputStream reader1 = new BufferedInputStream(new FileInputStream("primer_bmp.txt")); // потік
        ReadingHeaderFromBitmapImage reading = new ReadingHeaderFromBitmapImage(); // створення обєкту типу ReadingHeaderFromBitmapImage
        HeaderBitmapImage hbi = new HeaderBitmapImage(); // сворення об'єкту типу HeaderBitmapImage
